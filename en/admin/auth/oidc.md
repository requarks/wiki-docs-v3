...
| Use Discovery | Read the endpoints and signing keys from the issuer's `/.well-known/openid-configuration`. Turn off only for a provider that does not publish one, and fill in the endpoints below. | True |
...
| :mdi:subdirectory-arrow-right: Pass access token via GET query string to User Info Endpoint | Pass the access token in an `access_token` parameter attached to the GET query string of the User Info Endpoint URL. Otherwise the access token will be passed in the Authorization header. | False |
...
| Map Groups | Put the user in the wiki groups a claim names, on every login. Only groups that already exist here are matched, by name and ignoring case — nothing is created. | False |
| :mdi:subdirectory-arrow-right: Groups Claim | Which claim carries the group names. Either one name or a list of them. | `groups` |
| :mdi:subdirectory-arrow-right: Unassign from groups no longer present in claim | Off adds what the claim names and takes nothing away, so a membership granted here survives. On makes the provider the authority instead, and a group it stops naming is taken back — bar the ones this strategy auto-enrolls into, which are granted here to everyone it lets in. | False |
{.table-leading-col}
